...
Moved out to its own experiment, since it grew a script, a corpus and a set of measurements:
**[experiments/pdf_table_extraction/](experiments/pdf_table_extraction/README.md)**.

The short version. This is not on the current critical path — the corpus on disk is 40,534 XML
files and zero PDFs — but it is the route to two things nothing else reaches: user-uploaded papers
(Tier 2 above) and the opportunistic PDF sources that carry no tables, which includes Semantic
Scholar and most supplementary material (autonima#36, ~25% of otherwise-eligible studies).

Three findings worth keeping in view here:

- **There is a free, exact benchmark.** For any article whose coordinates we already hold parsed,
  those coordinates are ground truth for what an extractor should recover from its PDF. No hand
  labelling. 451 local articles; ~31,000 available from NeuroStore.
- **The LLM parser downstream changes the requirement.** What is needed is enough spatial fidelity
  for a model to reconstruct rows, not true table-structure recognition — a coordinate triple is
  numerically distinctive and tolerant of imperfect cell segmentation. So the no-model path should
  be tested before any model is bought, and there is no mature Rust equivalent of Docling anyway.
- **"Open access" and "obtainable" are different sets.** Entitled publisher APIs reach far more
  than free routes: Elsevier alone is 37-44% of the coordinate literature and returns 100%, while
  Semantic Scholar manages 19% against publisher bot-protection.
